...
final class CostDetailsViewController: UIViewController {
    ...
    private func updateUI(with product: Product) {
        ...
        currentPriceValueLabel.text = String(format: "%.3f", product.cost)
        priceTextField.text = String(format: "%.3f", product.cost)
    }
    
    private func updatePriceDisplay() {
        priceTextField.text = String(format: "%.3f", newPrice)
    }
    ...
    @objc private func confirmTapped() {
        ...
        ProductService.shared.updateCost(productId: product.id, newCost: newPrice) { [weak self] result in
            guard let self = self else { return }
            ...
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    print("Error updating cost: \(error.localizedDescription)")
                    self.showAlert(message: "Failed to update cost. Please try again.", shouldPopBack: false)
                }
            }
        }
    }
